...
<template>
    <div class="relative h-10 w-full rounded-full bg-lime-50">
        <div
            class="h-10 rounded-full bg-lime-200 transition-all duration-500 ease-in-out"
            :style="{ width: `${(step / todoList.length) * 100}%` }"
        ></div>
        <div class="absolute top-0 left-0 flex h-full w-full items-center justify-around px-3">
            <div
                class="inline-block h-6 rounded-full bg-lime-50 px-2 font-extrabold text-lime-500"
                :class="{
                    'opacity-50': index + 1 > step,
                }"
